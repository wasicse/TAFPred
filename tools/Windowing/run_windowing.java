...
public class run_windowing{
	...
	public static void main(String[] args) throws IOException {	
		...
		int num_of_features = 179; // you should provide exact number of features here without Target
		
		//	File currentDir = new File(new File(".").getAbsolutePath());
		//	String curr_dir_path = currentDir.getCanonicalPath();

		// BufferedReader brTest = new BufferedReader(new FileReader("../Dataset_Name.txt"));
    	// text = brTest .readLine();
		// FileReader fr = new FileReader("../Input/Dataset_Name.txt"); 
 		 // String Dataset_Name=brTest .readLine();
 		 // System.out.println(Dataset_Name);
		System.out.println(args[0]);
		String idListPath = "./"+args[0];
		String combinedFeatureDir = "./Merge_feat_phi_shifted/";
		String singleCysWindowedFilesDir = "./Windowed_file_JAVA_phi/";
		
		for(int ws = 1; ws <= 3; ws=ws+2){
			
			String singleCysWindowedFileName = singleCysWindowedFilesDir+"Feat_"+num_of_features+"_WS_"+ws+"_"+args[0]+"_.csv";
	...
	public static void createWindow(String idListPath, String combinedFeatureDir, String singleCysWindowedFileName, int window_size, int number_of_features) throws IOException {
		...
		while ((proteinID = listFileRd.readLine())!=null) {
			...
			while((featureLine = featureFileReader.readLine())!=null){
				...
				if(element[0].equalsIgnoreCase("Y_DPHI0")){
